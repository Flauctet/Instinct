...
/*:
 ...
 @param Force On PC
 @desc Appear in PC version (1 = YES; 0 = NO) 
 @default 0
...
(function ($) {
	...
	$.Param.forceOnPc = Number($.Parameters["Force On PC"]);
	...
	Sprite_DirPad.prototype.updateTouch = function() {
		...
		if (TouchInput.isPressed()) {
			...
			if (new PIXI.Rectangle(sx + 156, sy + 78, 78, 78).contains(TouchInput.x,TouchInput.y)) {
				Input._currentState['right'] = true;
				this._lastDir = 'right';
			} else if (new PIXI.Rectangle(sx, sy + 78, 78, 78).contains(TouchInput.x,TouchInput.y)) {
				Input._currentState['left'] = true;
				this._lastDir = 'left';
			} else if (new PIXI.Rectangle(sx + 78, sy, 78, 78).contains(TouchInput.x,TouchInput.y)) {
				Input._currentState['up'] = true;
				this._lastDir = 'up';
			} else if (new PIXI.Rectangle(sx + 78, sy + 156, 78, 78).contains(TouchInput.x,TouchInput.y)) {
	...
	Scene_Map.prototype.processMapTouch = function() {
		if (!isMobileDevice2()) Scene_Map_processMapTouch.apply(this, arguments);
	};
